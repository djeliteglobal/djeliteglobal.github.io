...
const CryptoPaymentPage: React.FC = () => {
  ...
  const createPayment = async () => {
    ...
    try {
      ...
      const response = await fetch(`${window.location.origin}/.netlify/functions/create-card-payment`, {
        method: 'POST',
    ...
    <div className="min-h-screen bg-gradient-to-br from-[color:var(--bg)] via-gray-900 to-black text-[color:var(--text-primary)]">
      <div className="max-w-md mx-auto px-6 py-16">
        <div className="text-center mb-8">
          ...
          <p className="text-[color:var(--text-secondary)]">Choose your plan and pay with card</p>
        </div>
        ...
        <div className="space-y-6">
          ...
          >
            {loading ? 'Processing...' : `Pay $${plans[selectedPlan].price} with Card`}
          </Button>
        </div>
      </div>
    </div>
